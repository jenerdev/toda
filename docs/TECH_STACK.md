...
| **Delivery** | PWA (`vite-plugin-pwa`) | Both commuters and drivers are on phones. Installable (custom install prompt + iOS "Add to Home Screen" fallback), no app store, one codebase. `registerType: 'prompt'` with a **"new version → Reload" banner** (`ReloadPrompt`/`useRegisterSW`) that **actively polls** for a new service worker (every 60s, on tab-visibility regain, and on regaining network) so the banner surfaces on its own; the Reload action reloads on the SW `controllerchange` with a 3s timed fallback so it works even on iOS, where the internal reload can misfire. A build id (`__BUILD_ID__`, the Vercel commit SHA, injected via Vite `define`) is shown in-app **and on the login page** to confirm the live version. Custom service-worker push handlers (`public/push-sw.js`) are merged into the generated Workbox SW via `workbox.importScripts`. |
...
| **Server state** | TanStack Query | Caching, refetch, and loading/error states around Supabase calls. Hooks expose `error`, and screens render shared `Spinner`/`Loading`/`EmptyState`/`ErrorState` primitives (`src/components/States.tsx`) — a failed fetch shows an error card with retry, never a misleading empty list. |
...
| **Auth** | Supabase Auth (**phone + OTP**, dummy `1234`) | Most users have no email/password, so the phone number is the login identifier with a one-time code. The phone maps to a synthetic email on a real public TLD (`<digits>@motoqueue.app`) + a derived hidden password — Supabase still issues a real session for RLS. OTP is a dummy `1234` for the MVP (swap in an SMS provider later). The login/signup OTP step has a **"Resend code" control with a 2-minute countdown** (`ResendOtp`), and signup gates "Send code" behind a **required Terms & Conditions agreement** (`TermsModal`). Requires **"Confirm email" OFF**. |
| **Realtime** | Supabase Realtime | Push ride offers to drivers and status changes to commuters with no polling. A reconnect banner warns when the socket drops (and refetches on recovery); ride completion shows a confirmation to both parties off the same channel. To control Realtime message/egress cost, broadcast-heavy signals are tuned: the no-drivers "any driver free?" count is **polled (20s)** rather than subscribed, the queue's `driver_states` refetch is **debounced (~1.5s)**, and live driver location rides a **participant-scoped** `driver_locations` channel (`0027`) instead of the broadly-readable `driver_states`. |
...
| **Routing** | **OSRM public demo** (keyless) | The commuter's live map draws the **road route** driver → pickup via `router.project-osrm.org` (`useRoute`), with a distance/ETA label. No API key. It's a **demo-grade** endpoint (rate-limited, best-effort), so it's throttled (refetch only ~per 100 m of driver movement) and **falls back to a straight line + great-circle distance** on any failure. |
...
> That boundary is hardened at the privilege layer too: client roles have direct `INSERT/UPDATE/DELETE` revoked on
> every RPC-managed table (`0026`), and `CREATE` on schema `public` is revoked from client roles (`0025`).
