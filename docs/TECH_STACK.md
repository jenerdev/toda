...
| **Realtime** | Supabase Realtime | Push ride offers to drivers and status changes to commuters with no polling. A reconnect banner warns when the socket drops (and refetches on recovery); ride completion shows a confirmation to both parties off the same channel. |
